...
class Enrollment:
    def __init__(self, subject, student, grade):
...
def enroll_to_subject(student, subject):
    if not isinstance(student, Student) or not isinstance(subject, Subject):
        return "Error"
    if search_enrollment_subject_student(subject, student) is None:
        enrollment_list.append(Enrollment(subject, student, None))
        return "Done"
    else:
...
def search_student_enroll_in_subject(subject):
    ...
    for enrollment in enrollment_list:
        if enrollment.subject == subject:
            selected_student_list.append(enrollment)
    return selected_student_list
...
def search_subject_that_student_enrolled(student):
    ...
    for enrollment in enrollment_list:
        if enrollment.student == student:
            selected_subject_list.append(enrollment)
    return selected_subject_list
...
def get_student_record(student):
    if not isinstance(student, Student):
        return "Error"
    selected_subject_list = search_subject_that_student_enrolled(student)
    record_dict = {}
    for enrollment in selected_subject_list:
        record_dict[enrollment.subject.subject_id] = [enrollment.subject.subject_name, enrollment.grade]
    return record_dict
...
def get_student_GPS(student):
    ...
    sum_credit_earned = 0
    sum_credit = 0
    for k, v in record.items(): #{ ‘subject_id’ : [‘subject_name’, ‘grade’ }
        subject_credit = search_subject_by_id(k).credit
        sum_credit += subject_credit
        sum_credit_earned += subject_credit * grade_to_count(v[1])
        
    return sum_credit_earned/sum_credit
